...
//부모 클래스(StarCraft)
//공통 메서드 life()를 정의.  자식 클래스들이 이를 오버라이드함.
//자식 클래스 life()메서드를 오버라이딩(재정의) 해서 고유한 동작을 정의.
//canon, run, fly 와 같은 각 클래스만의 고유 메서드를 추가.

//메인 클래스
//부모 클래스 배열 StarCraft[] 생성하여 다양한 자식 클래스를 저장
//다형성을 활용해 life()메서드를 호출하며 자식 클래스의 오버라이드된 메서드를 실행
